...
function MongoAggregator (url, options) {
	...
	self.chainCall(Db.connect.bind(Db, url), function (db) {
		self.chainCall(db.collectionNames.bind(db, options.collectionName), function (names) {
			if (names.length == 0 && !options.safe) {
				var collectionOptions = {capped: true, strict: true, size: options.size};
				self.chainCall(db.createCollection.bind(db, options.collectionName, collectionOptions), self.setCollection.bind(self));
			} else {
				self.chainCall(db.collection.bind(db, options.collectionName), self.setCollection.bind(self));
			}
		});
	});
};
...
MongoAggregator.prototype.setCollection = function (collection) {
	var self = this;
	self.chainCall(collection.options.bind(collection), function (collectionOptions) {
...
MongoAggregator.prototype.openCursor = function (collection) {
	...
	self.chainCall(latest.nextObject.bind(latest), function (latest) {
		...
		self.chainCall(cursor.each.bind(cursor), function(item) {
			if (item != null)
				self.emit('data', item);
			if (cursor.isClosed())
...
MongoAggregator.prototype.chainCall = function (callback, success) {
	var self = this;
...
MongoAggregator.prototype.listen = function (source) {
	...
	source.on('data', function (data) {
		...
		if (collection) {
			self.chainCall(collection.insert.bind(collection, data));
		} else {
