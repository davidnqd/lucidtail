...
function lucidtail (server, opts) {
	...
	var emitter = opts.aggregator || new lucidtail.Aggregator();
	emitter.pipe(lucidtail.emitter('socketio', server, {emitter: emitter, of: opts.of}));

	emitter.use = function() {
		var listened = lucidtail.emitter.apply(null, Array.prototype.slice.call(arguments));
		emitter.listen(listened);
		return emitter;
	}
