...
exports = module.exports = function (prepareServer, opts) {
	opts = opts || {};

	var socket = createSocket(prepareServer(prepareServer), opts.of);
 ...
 * Create a socket
 */
function createSocket(prepareServer, of) {
	var io = socketio(prepareServer);
 ...
 */
function prepareServer(prepareServer) {
	prepareServer = prepareServer || 8080;

	// Create a standard http prepareServer if one was not specified
	if (!isNaN(prepareServer)) {
		prepareServer = http.createprepareServer().listen(Number(prepareServer));
	}

	prepareServer.on('request', function httpRequestHandler (req, res) {
